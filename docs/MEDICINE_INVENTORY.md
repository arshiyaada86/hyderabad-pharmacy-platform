...
Existing product rows show locked medicine details; photograph changes use a dedicated photo-only update. Receive stock creates a new receipt with pricing and invoice attachment. Unit type is reused from the medicine/previous batch and cannot change once saved on the medicine. Legacy products without any recorded unit require one-time setup. Available quantity is a read-only total from the product, separate from the new receipt quantity. Publication can be chosen while creating a product.
