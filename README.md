...
- Home cycles through the three supplied greetings with grapheme-aware typing and reduced-motion support. Delivery locality no longer appends the city.
- Selected successful actions use an animated green check and auto-dismiss message; login, logout and adding cart items are silent. Adding items and changing cart quantities request native vibration where supported; iOS uses its system vibration duration. Order placement and medicine requests retain their persistent success message.
