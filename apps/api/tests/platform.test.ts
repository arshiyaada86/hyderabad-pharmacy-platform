...
test('batch receipts validate master data, keep costs private, allocate FEFO and cancel exactly once',async t=>{
 ...
 const firstBody=receipt();const first=await call('/admin/inventory/receive','POST',firstBody,admin);assert.equal(first.status,201,JSON.stringify(first.data));assert.equal(first.data.quantityAvailable,5);assert.equal(first.data.image,p.image);assert.equal(store.get('products',p.id)!.image,p.image);
