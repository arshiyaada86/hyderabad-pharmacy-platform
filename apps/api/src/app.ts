...
export function createApp(store:Store,options:Options){
 ...
 // Patients belong to an account; prescriptions belong to one patient, never inferred from the buyer.
 const customerExists=(id:string)=>store.get('customers',id)??fail(404,'Customer not found.');
 const patientData=z.object({name:z.string().trim().min(1).max(100),age:z.number().int().min(0).max(130),relationship:z.string().trim().max(60)}).strict();
 app.get('/api/admin/customers/:id/patients',(req,res)=>{permit(req,operational);customerExists(req.params.id as string);res.json({patients:store.list('patients').filter(p=>p.customerId===req.params.id),prescriptions:store.list('patientPrescriptions').filter(p=>p.customerId===req.params.id).sort((a,b)=>b.createdAt.localeCompare(a.createdAt))});});
 app.post('/api/admin/customers/:id/patients',(req,res)=>{const staff=permit(req,operational);customerExists(req.params.id as string);const data=patientData.parse(req.body);const p=store.transaction(()=>{const p=store.put('patients',{...data,customerId:req.params.id,ageRecordedAt:new Date().toISOString()});store.audit(staff.id,'add patient','patients',p.id,'Add household patient',null,p);return p;});res.status(201).json(p);});
 app.put('/api/admin/customers/:id/patients/:patientId',(req,res)=>{const staff=permit(req,operational);const input=z.object({data:patientData,version:z.number()}).strict().parse(req.body);const p=store.transaction(()=>{const old=own('patients',req.params.patientId as string,req.params.id as string);version(old,input.version);const p=store.put('patients',{...old,...input.data,ageRecordedAt:input.data.age===old.age?old.ageRecordedAt:new Date().toISOString()});store.audit(staff.id,'update patient','patients',p.id,'Correct household patient details',old,p);return p;});res.json(p);});
 app.post('/api/admin/customers/:id/patients/:patientId/prescriptions',(req,res)=>{
  const staff=permit(req,operational);const patient=own('patients',req.params.patientId as string,req.params.id as string);
  const input=z.object({title:z.string().trim().min(1).max(150),prescribedDate:z.string().regex(/^\d{4}-\d{2}-\d{2}$/),doctorName:z.string().trim().max(150),notes:z.string().trim().max(2000),photoId:z.string().min(1),orderId:z.string().optional()}).strict().parse(req.body);
  if(Number.isNaN(Date.parse(input.prescribedDate))||new Date(input.prescribedDate).toISOString().slice(0,10)!==input.prescribedDate||input.prescribedDate>new Date().toISOString().slice(0,10))fail(400,'Enter a valid prescription date, not in the future.');
  if(input.orderId)own('orders',input.orderId,patient.customerId);
  const upload=store.get('patientUploads',input.photoId);if(!upload||upload.patientId!==patient.id||upload.customerId!==patient.customerId)fail(400,'Upload a prescription for this patient.');
  mediaView(input.photoId,patient.customerId);
  const saved=store.transaction(()=>{if(store.list('patientPrescriptions').some(p=>p.photoId===input.photoId))fail(409,'This prescription image is already saved.');const p=store.put('patientPrescriptions',{...input,customerId:patient.customerId,patientId:patient.id,patientSnapshot:{name:patient.name,age:patient.age,ageRecordedAt:patient.ageRecordedAt},actor:staff.id});store.audit(staff.id,'save prescription','patientPrescriptions',p.id,'Save patient prescription',null,p);return p;});res.status(201).json(saved);
 });
 app.get('/api/admin/customers/:id/patients/:patientId/prescriptions/:prescriptionId/photo',(req,res)=>{const staff=permit(req,operational);const patient=own('patients',req.params.patientId as string,req.params.id as string);const prescription=own('patientPrescriptions',req.params.prescriptionId as string,patient.customerId);if(prescription.patientId!==patient.id)fail(404,'Prescription not found.');store.audit(staff.id,'view prescription','patientPrescriptions',prescription.id,'Review patient prescription');res.json(mediaView(prescription.photoId,patient.customerId));});
 ...
 app.post('/api/media',upload.single('file'),async(req,res)=>{
  const patientId=typeof req.query.patientId==='string'?req.query.patientId:'';let patient:RecordData|undefined;if(patientId){permit(req,operational);patient=store.get('patients',patientId)??fail(404,'Patient not found.');}
  const isPublic=req.query.public==='true',invoice=req.query.invoice==='true';if(patient&&(isPublic||invoice))fail(400,'Patient prescriptions must remain private.');if(invoice&&isPublic)fail(400,'Invoice images must remain private.');const owner=patient?customerExists(patient.customerId):isPublic||invoice?permit(req,['admin','catalog']):auth(req,'customer');if(!req.file)fail(400,'Select one image.');
  ...
  const id=randomUUID();store.db.prepare('INSERT INTO media VALUES(?,?,?,?,?,?)').run(id,owner.id,isPublic?1:0,bytes!,'image/jpeg',Date.now());if(patient)store.put('patientUploads',{id,patientId:patient.id,customerId:patient.customerId});res.status(201).json(mediaView(id,owner.id,isPublic));
 });
 ...
 app.delete('/api/media/:id',(req,res)=>{const user=auth(req,'customer');const row=store.db.prepare('SELECT owner FROM media WHERE id=?').get(req.params.id as string) as any;if(!row||row.owner!==user.id)fail(404,'Image not found.');if(store.list('patientPrescriptions').some(p=>p.photoId===req.params.id)||store.list('orders').some(o=>o.prescriptionId===req.params.id)||store.list('requests').some(r=>r.photoId===req.params.id))fail(409,'Submitted images cannot be removed as drafts.');store.db.prepare('DELETE FROM media WHERE id=?').run(req.params.id as string);res.json({ok:true});});
