...
export function receive(store:Store,raw:unknown,actor:string){
 ...
 return store.transaction(()=>{
  ...
  const prior=product;
  product=store.put('products',{...master,composition:d.genericName,image:'',imageAlt:d.brandName,status:'draft',mrpPaise:d.mrpPaise,pricePaise:d.pricePaise,...product,batchManaged:true,stock:0});
