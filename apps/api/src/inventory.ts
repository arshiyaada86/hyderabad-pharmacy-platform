...
export function receive(store:Store,raw:unknown,actor:string){
 ...
 return store.transaction(()=>{
  ...
  if(product?.unitType&&product.unitType!==d.unitType)bad('Unit type is fixed for this medicine.');
  ...
  const prior=product;
  product=store.put('products',{...master,composition:d.genericName,status:input.publication||'draft',mrpPaise:d.mrpPaise,pricePaise:d.pricePaise,...product,unitType:d.unitType,image:d.image,imageAlt:d.brandName,batchManaged:true,stock:0});
